...
function plotlc(fold, parameter, stat)
{
    ...
    var command = gnuplot +" -e \"set output 'lc/learning_curves/"+fold+"_"+parameter+".png'\" "+__dirname+"/lc.plot " + "-e \"plot for [i=3:"+(classifiers.length+2)+"] \'"+mapfile+"\' using 1:i:xtic(1) with linespoints linecolor i pt "+(fold == 'average' ? 0 : fold)+" ps 3, \'\' using 1:(NaN):x2tic(2) axes x2y1\" 2>>/tmp/gnuploterror" 
    console.log(command)
    child_process.execSync(command)
