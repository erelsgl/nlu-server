...
async.whilst(
    ...
    function (callbackwhilst) {
		...
		async.timesSeries(len+1, function(n, callbacktime){
			...
			console.log("worker "+process["pid"]+": index=" + index +" train="+train.length/classes.length+" testall="+test.length+" test="+test.length/classes.length +" length="+n+" maxlen="+len+" classifier="+classifier+" classes="+classes.length + " fold="+fold)			
