...
io.sockets.on('connection', function (socket) {
	...
	socket.on('translate', function (request) {
		...
		logger.writeEventLog("events", "-- translated", request);
		socket.emit('translation', classification);
		fs.appendFile(logger.cleanPathToLog("requests.log"), "AUTOMATIC "+classification.text + "  /  " +classification.translations.join(" AND ")+"\n");
	});
	
	
	socket.on('delete', function (request) {
		logger.writeEventLog("events", "DELETE", request);
	});
